...
public class Prisma extends FiguraGeo3D {
    ...
    public Prisma(double raio,double tamlado,int qtdlado, double alt,  Material material, String id) {
        setAlt(alt);
        setRaio(raio);
        setQtdlado(qtdlado);
        setTamlado(tamlado);
    ...
    public double getVolume() {
        if(qtdlado == 3){
        return (((raio*tamlado)/2)* this.getAlt());       
        }else{    
        return ((((raio*tamlado)/2)*qtdlado)* this.getAlt());
        }  
    }     
}
